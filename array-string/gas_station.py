...
class Solution(object):
    def canCompleteCircuit(self, gas, cost):
        total_tank = 0  # Total fuel left after completing the circuit
        curr_tank = 0  # Fuel in the tank while traveling
        start_station = 0
        ...
        for i in range(len(gas)):
            ...
            # If the tank is empty, reset the start station to the next station
            if curr_tank < 0:
                start_station = i + 1
                curr_tank = 0

        # If the total amount of fuel is non-negative, return the start station
        if total_tank >= 0:
